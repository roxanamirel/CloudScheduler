...
public class TurnOffRack  extends Action {
	private  float COST = 600;
	private Rack rack;
	public TurnOffRack(Server sourceServer, Server destinationServer,
			VirtualMachine vm,Rack rack) {
		super(sourceServer, destinationServer, vm,600*rack.getServers().size());
		this.rack = rack;
	...
	public DataCenter Do(DataCenter dc) {
	    for(Rack r:dc.getRack()){
	    	if(rack.getID() ==r.getID()){
	    		...
	    		for(Server server:r.getServers()){
